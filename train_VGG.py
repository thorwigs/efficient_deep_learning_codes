...
def collate_fn(batch):
    cutmix = v2.MixUp(num_classes=10)
    return cutmix(*default_collate(batch))

trainloader_cutmix = DataLoader(c10train_cutmix,batch_size=32,shuffle=True, collate_fn=collate_fn)
...
with wandb.init(project=project, config=config) as run:
    net = VGG("VGG16")
    path = "/homes/y23charo/Documents/effeicient_deep_learning/codes_lab1/stats/VGG16_100_mixup_1"
    stats = train(net, trainloader, testloader, "", run, **config)
    stats = train(net, trainloader_cutmix, testloader, path, run, stats, config["epochs"], **config)